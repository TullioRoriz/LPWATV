const LARGURA_ABELHA = 100;
const ALTURA_ABELHA = 100;
...
Abelhinha.prototype.posiciona = function(tempoAtrasoParaIniciar) {
  ...
  const UM_QUARTO_DA_ALTURA_DA_JANELA = window.innerHeight / 3;
  this.yInicial = 10;//Math.random(50) * (UM_QUARTO_DA_ALTURA_DA_JANELA - ALTURA_ABELHA);
  this.yFinal = 500; //Math.random(50) * (UM_QUARTO_DA_ALTURA_DA_JANELA - ALTURA_ABELHA);

  this.el.style.left = `${this.xInicial}px`;
  this.el.style.bottom = `${this.yInicial}px`;

  // duração da animação: de 3s até 6s (aleatório) 
  this.tempoTrajeto = 6000 + Math.random() * 6000;
